...
class Var(object):
    ...
    API_HASH = str(getenv('API_HASH', '9937930c368c669ca905e9a95aa712f0'))
    BOT_TOKEN = str(getenv('BOT_TOKEN' , '6465961232:AAGlS_LGkM-npZFUMH2jVHJhw6KtY2HCbdM'))
    PICS = (environ.get('PICS', 'https://envs.sh/jUp.jpg')).split()
    name = str(getenv('name', 'linkstreamrobot'))
    SLEEP_THRESHOLD = int(getenv('SLEEP_THRESHOLD', '60'))
    WORKERS = int(getenv('WORKERS', '4'))
    BIN_CHANNEL = int(getenv('BIN_CHANNEL', '-1002368112307'))
    NEW_USER_LOG = int(getenv('NEW_USER_LOG', '-1002368112307'))
    PORT = int(getenv('PORT', '8080'))
    BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', '0.0.0.0'))
    PING_INTERVAL = int(environ.get("PING_INTERVAL", "1200"))  # 20 minutes
    OWNER_ID = [int(x) for x in os.environ.get("OWNER_ID", "7040444713").split()]
    NO_PORT = bool(getenv('NO_PORT', False))
    APP_NAME = None
    OWNER_USERNAME = str(getenv('OWNER_USERNAME', 'LeoTGx_7'))
    if 'DYNO' in environ:
        ON_HEROKU = True
        ...
        ON_HEROKU = False
    FQDN = str(getenv('FQDN', 'BIND_ADRESS:PORT')) if not ON_HEROKU or getenv('FQDN', 'permanent-etti-leotg12345-d60c1491.koyeb.app/') else APP_NAME+'.herokuapp.com'
